 ...
 *  -> set default logging_enabled to true
 *  -> renamed log files stored on the sd card
...
void LogStr(const char *str)
{
    ...
    if (g_logging_enabled)
    {
        ...
        FILE *file = fopen("sdmc:/wlan.log", "ab+");
        if (file)
        {
    ...
    if (MemoryLogPos + len >= MemoryLogSize)
    {
        ...
        for (size_t i = 0; i < MemoryLogSize && MemoryLog[i] != '\0'; i++)
        {
            ...
            {
                dis = i + 1;
                if (MemoryLogPos - dis + len < MemoryLogSize)
                    break;
            }
        }
        if (dis)
...
Result SaveLogToFile()
{
    ...
    mutexLock(&MemoryLogMutex);
    FILE *file = fopen("sdmc:/wlan_memlog.log", "ab+");
    if (file)
    {
        fprintf(file, "wlan_logger memory log dump\nversion: " ATMOSPHERE_GIT_REVISION "\ntimestamp: %" PRIu64 "\n\n", curtime);
